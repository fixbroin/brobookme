...
export async function createBooking(
  ...
  if (isPaidService && onlinePaymentEnabled && data.paymentMethod === 'online') {
    ...
    // This now handles:
    // 1. Free bookings
    // 2. Paid bookings where "Pay After Service" is chosen
    // 3. Paid bookings where no payment method is enabled
    ...
    if (isPaidService && data.paymentMethod === 'later') {
        params.set('amountPaid', '0'); // Explicitly set to 0 for Pay Later confirmation
...
export async function updateProviderSubscription(
  ...
  try {
    ...
    if (isRenewal) {
        // Could be a renewal or an upgrade
